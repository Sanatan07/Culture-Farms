      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {products.map((product, index) => (
            ...
            >
              <div className={`w-full h-48 ${product.bgColor} rounded-xl mb-6 flex items-center justify-center`}>
                <Leaf className={`w-20 h-20 ${product.iconColor}`} />
              </div>
              <h3 className="text-2xl font-bold text-green-800 mb-3">{product.name}</h3>
              <p className="text-gray-600 mb-4 leading-relaxed">
                {product.description}
              </p>
              <span className={`inline-block ${product.badgeColor} text-white px-4 py-2 rounded-full text-sm font-medium`}>
